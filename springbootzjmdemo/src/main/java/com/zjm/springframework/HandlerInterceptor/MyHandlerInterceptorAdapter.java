...
import com.zjm.springframework.Listener_Filter_Initializer_wrapper.CustomHttpRequestWrapper;
...
public class MyHandlerInterceptorAdapter extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (path.contains("test2")) {
            String paramString = path.substring("/baseapplication/springBootBase/test2/".length(), pathSize);
        ...
        if (path.contains("test4")) {
            ...
            request.getRequestDispatcher("/baseapplication/springBootBase/requestForward").forward(request, response);
            return false;
        }
        return true;
